<script setup>
    ...
    const rules = computed(() => ({
        ...
        emailField: {
            email: helpers.withMessage('Вы ввели некоректний email', email)
        },
        luckyField: {
            maxLength: helpers.withMessage(`Максимальна довжина: 2 символи`, maxLength(3)),
            numeric: helpers.withMessage('Вы можете ввести лише цифри', numeric)
        },
        confirmPasswordField: {
            sameAsPassword: helpers.withMessage(`Паролі не співпадають`, sameAs(passwordField.value))
        },
        frontendField: {
            frontendField: helpers.withMessage('Рядок повинен містити слово frontend', mustBeFrontend)
        }
    }))
